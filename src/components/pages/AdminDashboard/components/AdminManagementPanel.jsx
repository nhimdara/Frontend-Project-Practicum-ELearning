    ...
    <div className="grid gap-5 lg:grid-cols-[.8fr_1.2fr]">
      <form onSubmit={createAdmin} className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
        ...
        <div className="mt-5 space-y-4">
          ...
          <label className="block text-xs font-semibold text-slate-400">Generated temporary password<div className="mt-1.5 flex gap-2"><input required readOnly type="text" minLength={8} className={`${fieldClass} mt-0 font-mono`} value={form.password} placeholder="Generated from full name" /><button type="button" onClick={() => setForm({ ...form, password: generatePassword() })} className="admin-utility-icon-button rounded-xl p-3" title="Generate another password" aria-label="Generate another password"><RefreshCw className="h-4 w-4" /></button><button type="button" onClick={() => navigator.clipboard?.writeText(form.password)} disabled={!form.password} className="admin-utility-icon-button rounded-xl p-3 disabled:opacity-40" title="Copy password" aria-label="Copy password"><Copy className="h-4 w-4" /></button></div><span className="mt-1 block font-normal text-amber-400">Copy this password before creating the account.</span></label>
